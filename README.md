...
Этап 1. Создается бакет [S3](./stage1/s3_ph1.tf) и таблица [YDB](./stage1/ydb_ph1.tf) с локальным хранением файла состояния. Также создается отдельный [сервисный аккаунт](./stage1/service_account_k8s_sa_ph1.tf) с нужными правами для инфораструктуры и скачивания тестового образа - [image-puller](./stage1/puller-sa.tf)
